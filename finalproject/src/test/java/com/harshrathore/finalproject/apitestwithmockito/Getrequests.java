...
private static String url  = "http://localhost:8080/bms/77";
	@Test
	void getRequestNotFoundTest() throws Exception {
		String op = """
				{
    "maker_name":"KTM",
    "model_name":"RC390",
	...
	@Test
	void getRequestFound() throws Exception {
		String op = """
				{
    "maker_name":"KTM",
    "model_name":"RC390",
    ...
    "phone_number":"7021687790"
		...
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get("http://localhost:8080/bms/1").accept(MediaType.APPLICATION_JSON);
		...
		when(service.getBike(1)).thenReturn(bike);
	    MvcResult result = mockMvc.perform(request).andReturn();
	    
		MockHttpServletResponse response = result.getResponse();
		assertEquals(200 ,response.getStatus());
		JSONAssert.assertEquals(op, response.getContentAsString(), false);
	}
